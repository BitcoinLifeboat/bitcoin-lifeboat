...
The active GitHub org is configured for the public repository. Two release-signing
values still ship as **placeholder tokens** because they depend on the maintainer.
Some release/package templates may also carry placeholder-shaped values until concrete
release artifacts are cut. Builds proceed normally with the placeholders in place — you
can develop, test, and cut `alpha`/`beta` releases without replacing them. **US-099
blocks any public (non-alpha/beta) release while any placeholder token remains in the
tree.**
...
| --- | --- | --- |
| `github.org` | `BitcoinLifeboat` | GitHub organization or user that owns the repo. Feeds the external-link allowlist and release URLs. |
| `github.repo` | `bitcoin-lifeboat` | Repository name. |
| `github.url_base` | `https://github.com/BitcoinLifeboat/bitcoin-lifeboat` | Base GitHub URL. Built from `org` + `repo`; the link allowlist (US-043) is generated from this. |
...
| `placeholders.tokens` | see file | Extra placeholder-shaped tokens US-099 scans for before allowing a public release. The release-signing placeholder tokens are enforced by the gate. |

## Placeholder tokens

| Token | Replace with | Required before |
| --- | --- | --- |
| `__GH_ORG__` | Remaining release/package template references | Public (stable) release |
...
1. Edit `project.config.toml` and set `maintainer.pgp_fingerprint` and
   `maintainer.pgp_key_block_file` to real values.
