// Bitcoin Lifeboat — owner runbook: multisig-2of3 (US-032).
//
// Renders the sixteen §9.5 required sections for a 2-of-3 multisignature
// wallet, in public-safe (default) or private export mode, with labeled blank
// fields. The signer/device checklist (7) lists each cosigner's fingerprint and
// path (xpubs are never printed); the wallet-type summary (5) and verification
// steps (11) reflect the 2-of-3 quorum. The sections, blocks, and the optional
// §14.3 privacy banner are built by runbook-engine's `owner` module and
// delivered verbatim as `data.sections` / `data.privacy_banner` (the single
// source of truth shared with the Markdown and printpdf renderers). Reads
// everything from `data.json`; nothing from the clock or environment, so the
// render is reproducible (the engine also sets SOURCE_DATE_EPOCH=0). Inter /
// JetBrains Mono come via `--font-path`.
//
...
Recovery runbook for a 2-of-3 multisignature wallet.
