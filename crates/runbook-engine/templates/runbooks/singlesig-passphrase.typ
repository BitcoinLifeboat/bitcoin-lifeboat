// Bitcoin Lifeboat — owner runbook: singlesig-passphrase (US-032).
//
// Renders the sixteen §9.5 required sections for a single-signature wallet that
// is protected by a BIP39 passphrase, in public-safe (default) or private
// export mode, with labeled blank fields. The passphrase section (8) reminds
// the owner that a passphrase EXISTS but never records its value. The sections,
// blocks, and the optional §14.3 privacy banner are built by runbook-engine's
// `owner` module and delivered verbatim as `data.sections` /
...
Recovery runbook for a single-signature wallet protected by a passphrase.
